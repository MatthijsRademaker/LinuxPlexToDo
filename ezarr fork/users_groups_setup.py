...
class UserGroupSetup:
    def __init__(self, root_media_dir='/', root_config_dir='/'):
        self.root_media_dir = root_media_dir
        ...
        os.system(
            '/bin/bash -c "sudo mkdir -pv ' + self.root_media_dir + '/data/{media,usenet,torrents} -m 775'
            ' ; sudo chown $(id -u):mediacenter ' + self.root_media_dir + '/data' +
          ' ; sudo chown $(id -u):mediacenter ' + self.root_media_dir + '/data/{media,usenet,torrents}"'
        )

    def create_config_dir(self, service_name):
        os.system(
            f'sudo mkdir -p {self.root_config_dir}/config/{service_name}-config -m 775' # -m 775 gives read/write access to the whole mediacenter group, change to 755 for only read
            f' ; sudo chown -R {service_name}:mediacenter {self.root_config_dir}/config/{service_name}-config'
            f' ; sudo chown $(id -u):mediacenter {self.root_config_dir}/config'
        )

    def sonarr(self):
        os.system(
            '/bin/bash -c "sudo useradd sonarr -u 13001'
            ' ; sudo mkdir -pv ' + self.root_media_dir + '/data/{media,usenet,torrents}/tv -m 775'
            ' ; sudo chown -R sonarr:mediacenter ' + self.root_media_dir + '/data/{media,usenet,torrents}/tv"'
        )
    ...
    def radarr(self):
        os.system(
            ...
            ' ; sudo mkdir -pv ' + self.root_media_dir + '/data/{media,usenet,torrents}/movies -m 775'
            ' ; sudo chown -R radarr:mediacenter ' + self.root_media_dir + '/data/{media,usenet,torrents}/movies"'
        )
    ...
    def lidarr(self):
        os.system(
            ...
            ' ; sudo mkdir -pv ' + self.root_media_dir + '/data/{media,usenet,torrents}/music -m 775'
            ' ; sudo chown -R lidarr:mediacenter ' + self.root_media_dir + '/data/{media,usenet,torrents}/music"'
        )
    ...
    def readarr(self):
        os.system(
            ...
            ' ; sudo mkdir -pv ' + self.root_media_dir + '/data/{media,usenet,torrents}/books -m 775'
            ' ; sudo chown -R readarr:mediacenter ' + self.root_media_dir + '/data/{media,usenet,torrents}/books"'
        )
    ...
    def mylar3(self):
        os.system(
            ...
            ' ; sudo mkdir -pv ' + self.root_media_dir + '/data/{media,usenet,torrents}/comics -m 775'
            ' ; sudo chown -R mylar:mediacenter ' + self.root_media_dir + '/data/{media,usenet,torrents}/comics"'
        )
    ...
    def audiobookshelf(self):
        os.system(
            ...
            ' ; sudo mkdir -pv ' + self.root_media_dir + '/data/media/{audiobooks,podcasts,audiobookshelf-metadata} -m 775'
            ' ; sudo chown -R audiobookshelf:mediacenter ' + self.root_media_dir + '/data/media/{audiobooks,podcasts,audiobookshelf-metadata}"'
        )
    ...
    def qbittorrent(self):
        ...
        self.create_config_dir('qbittorrent')
